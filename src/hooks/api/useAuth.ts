...
export function useLogin() {
  ...
  return useMutation({
    mutationFn: async (credentials: LoginCredentials) => {
      const { data } = await api.post<ApiResponse<LoginResponse | OTPRequiredResponse>>(
        "/auth/login",
        credentials
      );
      return data; // Return the inner data directly
    },

    onSuccess: (data: any) => {
      // If tokens are returned, login immediately (OTP bypassed)
