...
import { useGenerateQR } from '@/hooks/api/useQRCode';
...
export default function GenerateQRDialog({ isOpen, onClose }: GenerateQRDialogProps) {
  const [startDate, setStartDate] = useState('');
  const [endDate, setEndDate] = useState('');
  const [shift, setShift] = useState<'morning' | 'afternoon' | 'all_day'>('all_day');
  ...
  const handleGenerate = async () => {
    ...
    try {
      const response = await generateQR.mutateAsync({
        startDate,
        endDate,
        shift,
        type,
        ...
        <body>
          <div class="container">
            ...
            <div class="date-range">
              Hiệu lực: ${format(new Date(startDate), 'dd/MM/yyyy', { locale: vi })} - ${format(new Date(endDate), 'dd/MM/yyyy', { locale: vi })}
            </div>
  ...
  return (
    ...
    >
      <div className="w-full max-w-2xl rounded-lg bg-white p-6 shadow-xl dark:bg-gray-900 animate-in fade-in zoom-in duration-200">
        {/* Header */}
          ...
          <div className="space-y-6">
            ...
            <div className="rounded-lg bg-blue-50 p-4 dark:bg-blue-900/10">
              <p className="text-sm text-blue-800 dark:text-blue-300">
                <strong>Lưu ý:</strong> QR code sẽ chỉ hiệu lực trong khoảng thời gian và ca làm việc đã chọn.
              </p>
            </div>
          ...
          <div className="space-y-6">
            ...
            <div className="flex flex-col items-center justify-center rounded-lg border-2 border-dashed border-gray-300 bg-gray-50 p-8 dark:border-gray-600 dark:bg-gray-800/50">
              ...
              <div className="mt-4 text-center">
                ...
                <p className="mt-1 text-sm text-gray-600 dark:text-gray-400">
                  {generatedQR.shift === 'morning' ? 'Ca Sáng (06:00 - 08:30)' : 
                   generatedQR.shift === 'afternoon' ? 'Ca Chiều (13:30 - 14:30)' : 'Cả ngày'}
                  {' • '}
            ...
            <div className="rounded-lg bg-green-50 p-4 dark:bg-green-900/10">
              <p className="text-sm text-green-800 dark:text-green-300">
                ✅ QR code đã được tạo thành công! Bạn có thể tải xuống hoặc in ra để nhân viên quét.
              </p>
            </div>
          </div>
        )}
