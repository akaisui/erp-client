        ...
        <table className="w-full text-sm">
          ...
          <tbody className="divide-y divide-gray-200 dark:divide-gray-700">
            {sortedData.map((item, index) => {
              const isExpanded = expandedRows.has(`${item.productId}-${item.warehouseName}`);
              const status = statusConfig[item.status];
              const uniqueKey = `${item.productId}-${item.warehouseName || index}`;
                  ...
                  <tr className={`hover:bg-gray-50 dark:hover:bg-gray-700/50 transition-colors ${status.bg}`}>
                    ...
                    <td className="px-6 py-4 text-center">
                      <div className="flex items-center justify-center gap-2">
                        ...
                        <button
                          onClick={() => toggleExpandRow(`${item.productId}-${item.warehouseName}`)}
                          className="rounded p-1 hover:bg-gray-200 dark:hover:bg-gray-600"
                          title="Mở rộng"
                        >
                          <ChevronDown
